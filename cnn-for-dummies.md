...
we have
\\[ \\begin{array}{rlr}
\\qquad \\quad & \\frac{\\partial z}{\\partial (vec(\\mathbf{x}^l)^T)} = vec\\left( \\frac{\\partial z}{\\partial Y} F^T \\right)^TH, & \\qquad \\; (43)
\\end{array} \\]
or equivalently
\\[ \\begin{array}{rlr}
...
In order to pinpoint one element in \\( vec(\\mathbf{x}^l) \\) or one row in H T , we need an
